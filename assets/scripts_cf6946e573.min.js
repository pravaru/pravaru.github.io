...
$(document).ready(function() {
    ...
    if ($('#sli-text-type').length) {
    let typed = new Typed('#sli-text-type', { 
        strings: ["Купить права"],
        typeSpeed: 200,
        startDelay: 500,
        backSpeed: 50,
        loop: true
    ...
    //Мобильное меню
    $(document).on('click','.menu-mobile:not(.menu-active)',function(e){
        $('header .main-menu nav').slideDown();
        $(this).addClass("menu-active");
    });
    $(document).on('click','.menu-mobile.menu-active',function(e){
        $('header .main-menu nav').slideUp();
    ...
    //Pre send data
    $(document).on('click','.ajax-pre-send',function(e){
        setTimeout(function() {
            /*grecaptcha.render('g-recaptcha', {
              'sitekey' : '6LexD9wZAAAAAHVBwyD9i3DpV2042PuDAUsyiNCL'
            });*/
            var endTime = new Date();
            var secondsTime = (endTime - startTime) / 1000;
            var urlPage = window.location.href;
            urlPage = urlPage.replace(new RegExp("\/",'g'),"__");
            $('[name="purl"]').val(urlPage);    
            $('[name="time"]').val(secondsTime);
        }, 1000);
        $('div.ajax-btn-wrap').html('<span class="btn ajax-send">Отправить</span>');
    });

    //Modal content
    $(".ajax-open").click(function(){
        var loadUrl = $(this).data("load");
        if(loadUrl === undefined) loadUrl = 'order';
        var endTime = new Date();
        var secondsTime = (endTime - startTime) / 1000;
    ...
    //Hidden block
    $(document).on('click','.hidden-show',function(e){
        $(this).hide();
        $('div.regions-top').hide();
    ...
    $(document).on('click','.ajax-send',function(e){
        ...
        var captcha = 1;
        sendData = sendData + '&g-recaptcha-response=' + captcha;
            ...
            $.ajax({
                ...
                success: function(json, textStatus) {
                    if (json.error.length > 3) {
                        ...
                        //$.magnificPopup.close();
                        $('div.modal-win #fform').remove();
                        $('div.modal-win div.container-fluid').html(json.result);
                        $('#page-form').remove();
                        $('div.col-lg-7').html(json.result);
                    }
                },
    ...
    $(document).on('click','.ajax-close',function(e){
        $.magnificPopup.close();
        $('section.form-on-page-wrap').slideUp();
    });

    //Send File
    $( document ).on( 'click', '.file-send', function() {
        if(!$('[name="phone"]').val()) {
            ShowMes('Укажите номер телефона!');
            return false;
        }

        $("#fform").append('<div class="loader loader-default is-active" data-half></div>');

        //if (document.getElementById("fform").file1.value !== '') {

                var form = document.forms.fform;
                var formData = new FormData(form);
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "/ajax/ajax-send-attach");
                xhr.onreadystatechange = function () {
                    if (xhr.readyState == 4) {
                        if (xhr.status == 200) {
                            ...
                            if (data === 'send') {
                                $("#fform").trigger('reset');
                                $("#fform").html('<h3>Сообщение отправлено!</h3><p>В течении 10 минут вам перезвонит наш специалист для уточнения деталей заказа</p><br><div class="text-center"><span class="btn ajax-close">OK</span></div>');
                            } else if (data === 'bot') {
                                ShowMes('Подтвердите что вы не робот');
                                $("#fform .loader").remove();
                            } else {
                                ShowMes('Ошибка, проверьте заполненые поля');
                                $("#fform .loader").remove();
                            }
                        }
                    }
    ...
    //Dropdown
    if($(window).width() > 991) {
        $('.main-menu ul.menu-start > li').mouseenter(function(){
            $(this).find('div').stop(true);
            $(this).find('div').slideDown();
        });
        $('.main-menu ul.menu-start > li').mouseleave(function(){
    ...
    //Подгрузка скриптов для ускорения
    if($('#lazy-load-block').length) {
        setTimeout(function() {                    
            $.ajax({
                url: "/ajax/lazy-load",
                dataType: 'json',
                success: function(data){
                    $('#lazy-load-block').html(data);
                }
            });
        }, 3000);
    }
